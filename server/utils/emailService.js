...
// Send booking completed email to host – "Your Earnings Are On The Way!"
const sendBookingCompletedEarningsEmail = async (host, booking, vehicle) => {
  try {
    if (!isEmailConfigured()) {
      console.log(`📧 [DEV] Booking Completed Earnings Email to: ${host.email}`);
      return { success: true, dev: true };
    }
    ...
    const vehicleImageUrl = getVehicleImageUrl(vehicle);
    const vehicleName = `${vehicle.year} ${vehicle.make} ${vehicle.model}`;
    const earnings = Number(booking.hostEarnings) || Number(booking.totalPrice) || 0;

    const mailOptions = {
      to: host.email,
      subject: `Trip Complete - $${earnings.toFixed(2)} Earnings On The Way!`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 30px 20px; text-align: center; border-radius: 8px 8px 0 0; }
            .content { background: #f9fafb; padding: 30px; border-radius: 0 0 8px 8px; }
            .earnings-card { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981; }
        ...
        <body>
          <div class="container">
            <div class="header">
              <h1>Your Earnings Are On The Way!</h1>
              <p style="margin: 0; opacity: 0.9;">Trip completed successfully</p>
            </div>

            <div class="content">
              <h2>Hi ${host.firstName},</h2>
              <p>Great news! The trip for your <strong>${vehicleName}</strong> has been completed and your earnings are on their way!</p>

              <div class="earnings-card">
                ${vehicleImageUrl ? `
                <div style="text-align: center; margin-bottom: 15px;">
                  <img src="${vehicleImageUrl}" alt="${vehicleName}" style="max-width: 100%; height: auto; max-height: 200px; border-radius: 8px; object-fit: cover;" />
                </div>
                ` : ''}
                <div style="background: #f0fdf4; padding: 10px 15px; border-radius: 6px; margin-bottom: 15px; text-align: center;">
                  <span style="color: #6b7280; font-size: 0.85rem;">Reservation ID</span><br>
                ...
                <h3 style="margin-top: 0; color: #10b981;">${vehicleName}</h3>
                ...
                <div class="amount-display">$${earnings.toFixed(2)}</div>
                <p style="text-align: center; color: #6b7280; margin: 0;">Your earnings for this trip</p>
              </div>

              <div class="info-box">
                <h4 style="margin-top: 0; color: #059669;">When will I get paid?</h4>
                <p style="margin: 5px 0;">Your earnings are now eligible for payout. They will be included in the next weekly payout cycle (every Monday). You can track all your earnings from your dashboard.</p>
              </div>
...
Great news! The trip for your ${vehicleName} has been completed and your earnings are on their way!
...
Your earnings are now eligible for payout. They will be included in the next weekly payout cycle (every Monday). You can track all your earnings from your dashboard.

View your bookings: ${clientUrl}/host/bookings
...
The RentUFS Team
    ...
    const result = await sendEmail(mailOptions);
    if (result.success) {
      console.log(`✅ Booking completed earnings email sent to host: ${host.email} ($${earnings.toFixed(2)})`);
    }
    return result;
  } catch (error) {
    console.error('❌ Error sending booking completed earnings email:', error);
    return { success: false, error: error.message };
  }
};
...
module.exports = {
  ...
  sendBookingCompletedEarningsEmail,
