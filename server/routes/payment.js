...
// Create payment intent for vehicle switch charge (driver-initiated from their bookings)
router.post('/create-vehicle-switch-payment', auth, async (req, res) => {
  try {
    ...
    // Only the driver can pay their own switch charge
    if (booking.driver._id.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'Unauthorized' });
    }

    const pendingAmount = booking.pendingSwitchCharge || 0;
    if (pendingAmount <= 0) {
      return res.status(400).json({ message: 'No pending switch charge on this booking' });
    }
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      description: `Vehicle switch charge: ${booking.vehicle.year} ${booking.vehicle.make} ${booking.vehicle.model} (Booking ${booking.reservationId})`
    });
...
// Confirm vehicle switch payment (driver pays the pending charge)
router.post('/confirm-vehicle-switch-payment', auth, async (req, res) => {
  try {
    ...
    const paidAmount = booking.pendingSwitchCharge || 0;
    booking.pendingSwitchCharge = 0;

    // Update the last switch history entry with the payment ID
    if (booking.vehicleSwitchHistory?.length > 0) {
      const lastSwitch = booking.vehicleSwitchHistory[booking.vehicleSwitchHistory.length - 1];
      if (!lastSwitch.paymentId) {
        lastSwitch.paymentId = paymentIntentId;
      }
    }

    // Update processing fees from the supplemental charge
    const rentalPortion = paidAmount / (1 + 0.029 / (2 - 0.029)); // Reverse the processing fee
    const supplementalProcessing = calculateProcessingFee(rentalPortion);
    ...
    // Recalculate host earnings with new processing fee
    const newBaseRental = booking.pricePerDay * booking.totalDays;
    ...
    console.log(`✅ Vehicle switch charge paid for booking ${booking.reservationId}: $${paidAmount.toFixed(2)}`);

    res.json({
      success: true,
      message: 'Vehicle switch payment completed!',
      booking: {
        _id: booking._id,
        reservationId: booking.reservationId,
        paidAmount,
        pendingSwitchCharge: 0
      }
    });
  } catch (error) {
  ...
  switch (event.type) {
    case 'payment_intent.succeeded': {
      ...
      try {
        ...
        // Check if this is a vehicle switch payment (driver paying pending charge)
        if (paymentIntent.metadata?.type === 'vehicle_switch') {
          if (existingBooking && existingBooking.pendingSwitchCharge > 0) {
            // Clear the pending charge
            existingBooking.pendingSwitchCharge = 0;

            // Tag the last switch history entry with the payment ID
            if (existingBooking.vehicleSwitchHistory?.length > 0) {
              const lastSwitch = existingBooking.vehicleSwitchHistory[existingBooking.vehicleSwitchHistory.length - 1];
              if (!lastSwitch.paymentId) {
                lastSwitch.paymentId = paymentIntent.id;
              }
            }

            await existingBooking.save();
            console.log(`✅ Vehicle switch charge paid via webhook for booking ${bookingId}`);
          }
          break;
        }
